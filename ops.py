...
def get_data_moving_MNIST(args):
    ...
    with tf.device(
        ...
        if args.train:
            print('Train files:', glob.glob(os.path.join(args.data_path , 'train.tfrecord')))
            files = tf.data.Dataset.list_files(glob.glob(os.path.join(args.data_path , 'train.tfrecord')))
        else:
            print('Train files:', glob.glob(os.path.join(args.data_path, 'test.tfrecord')))
...
def make_tensorboard(x_in, im_out, loss, args):
    ...
    tf.summary.scalar('loss L1',tf.reshape(loss,[]))
