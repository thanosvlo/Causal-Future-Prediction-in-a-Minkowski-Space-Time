...
def info_objective(model, data, K=1, beta=1.0, components=False, analytical_kl=False, **kwargs):
    """Computes E_{p(x)}[ELBO] """
    # for frame in range(0,4):
    labels = kwargs.get('labels')
    args = kwargs.get('args')
    info_zs = []
    for i in range(args.pos_samples+args.neg_samples):
        _, _, _zs = model(data[:,i,...], K)
        info_zs.append(_zs)
