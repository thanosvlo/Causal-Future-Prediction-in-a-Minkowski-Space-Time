...
class PoincareBall(PoincareBallParent):
    ...
    def to_poincare_ball_from_lorentz(self, u):
        x = u.clone()
        d = x.size(-1) - 1
        return x.narrow(-1, 1, d) / (x.narrow(-1, 0, 1) + 1)

    def from_poincare_ball_to_lorentz(self, u):
        x = u.clone()
        temp = (1 + (x.norm(dim=-1))**2)
        top = torch.cat((temp.unsqueeze(-1),2*x),-1)
        bottom = (1 - (x.norm(dim=-1))**2)
