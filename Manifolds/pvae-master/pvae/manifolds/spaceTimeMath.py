...
class SpaceTimeManifold(Manifold):
    ...
    def inner(self,x, u, v, c=self.c, keepdim=keepdim, dim=dim):
    ...
    def dist(self, x, y, c=self.c, keepdim=keepdim, dim=dim):
    ...
    def distance(self, u, v):
        ...
        d.data.clamp_(min=1)
        return self.acosh(d, self._eps)

    def norm(self, u):
        return th.sqrt(th.sum(th.pow(self.to_poincare_ball(u), 2), dim=-1))

    def project(self, x, c=None, dim= None):
        norm = self.norm(x).clamp_min(MIN_NORM)
    ...
    def expmap(self, x, u, c=self.c, dim=dim):
        return self.expm(x, u,)
