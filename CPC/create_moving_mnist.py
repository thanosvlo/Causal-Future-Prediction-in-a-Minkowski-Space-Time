...
def load_dataset(training=True,labels=True):
    ...
    if labels:
        if training:
            return load_mnist_images('train-images-idx3-ubyte.gz'), load_mnist_images('train-labels-idx1-ubyte.gz')
        return load_mnist_images('t10k-images-idx3-ubyte.gz'), load_mnist_images('t10k-labels-idx1-ubyte.gz')
...
def generate_moving_mnist(training, shape=(64, 64), num_frames=30, num_images=100, original_size=28, nums_per_image=2):
    ...
    '''
    mnist,labels = load_dataset(training)
    width, height = shape
    ...
    _labels = np.empty((num_frames * num_images, 1), dtype=np.uint8)
    ...
    for img_idx in range(num_images):
        ...
        mnist_images = [(Image.fromarray(get_image_from_array(mnist, r, mean=0)).resize((original_size, original_size),
                                                                                       Image.ANTIALIAS), labels[r]) \
                        for r in np.random.randint(0, mnist.shape[0], nums_per_image)]
        _label = mnist_images[0][1]
        mnist_images = mnist_images[0][0]
        ...
        for frame_idx in range(num_frames):
            ...
            for i, canv in enumerate(canvases):
                canv.paste(mnist_images, tuple(positions[i].astype(int)))
            ...
            _labels[img_idx * num_frames + frame_idx] = _label
...
def main(training, dest, filetype='npz', frame_size=64, num_frames=30, num_images=100, original_size=28,
         nums_per_image=2):
    dat, labels = generate_moving_mnist(training, shape=(frame_size, frame_size), num_frames=num_frames, num_images=num_images,
                                original_size=original_size, nums_per_image=nums_per_image)
    n = num_images * num_frames
    if filetype == 'npz':
        np.savez(dest, dat)
        np.savez(dest+'_labels',labels)
    elif filetype == 'jpg':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dest', type=str, dest='dest', default='./moving_mnist_train_with_labels')
    ...
    parser.add_argument('--num_images', type=int, dest='num_images', default=30000)  # number of sequences to generate
